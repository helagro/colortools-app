...
class ColorConvert {
    ...
    static String ColorIntToString(int selectedId, int color) {
        switch (selectedId) {
            case R.id.color_hex_btn:
                return isOpaque(color) ?
                        String.format("#%06X", (0xFFFFFF & color)) : String.format("#%08X", color);
            case R.id.color_rgb_btn:
                return getAlpha(color, true)
                        + red(color) + " "
                        + green(color) + " "
                        + blue(color)
                        + getAlpha(color, false);
            case R.id.color_int_btn:
    ...
    private static String getAlpha(int color, boolean first) {
        if (!isOpaque(color) && first == MainActivity.is_argb) {
            ...
            final String alphaString = MainActivity.byte_alpha ? Integer.toString(alpha) : String.format(Locale.US, "%.2f", alpha / 255f);
